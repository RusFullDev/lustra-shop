<script setup></script>
<template>
  <div class="bg-foot">
    <div class="container py-12 md:py-20">
      <div class="md:flex gap-[150px] mb-6">
        <div class="mb-8">
            <nuxt-link to="/">
                <div class="flex gap-3 items-center mb-[50px]">
                  <img src="/footer/Vector.png" alt="Logo" />
                  <img src="/footer/NORNLIGHT.png" alt="Logo" />
                </div>
            </nuxt-link>
          <p class="mb-9 font-gray font-medium text-text-2xl">
            8 (800) 890-46-56
          </p>
       <nuxt-link to="https://cis.visa.com/"><img src="/footer/image 1.png" alt="visa card" class="mb-9" /></nuxt-link>   
          <p class="mb-8 text-grayn/50 font-medium text-base">
            Политика конфидециальности
          </p>
          <p class="text-grayn/50 font-medium text-base">
            Пользовательское соглашение
          </p>
        </div>

        <div class="mb-8">
          <p class="font-garyn font-bold text-2xl pb-8">Покупателям</p>
          <nuxt-link to="/about"
            ><p class="text-grayn/50 font-medium text-base mb-7">
              О компании
            </p></nuxt-link
          >
          <nuxt-link to="/shiping-payment"
            ><p class="text-grayn/50 font-medium text-base mb-7">
              Доставка и оплата
            </p></nuxt-link
          >
          <nuxt-link to="/return"
            ><p class="text-grayn/50 font-medium text-base mb-7">
              Возврат
            </p></nuxt-link
          >
          <nuxt-link to="/garant"
            ><p class="text-grayn/50 font-medium text-base mb-7">
              Гарантии
            </p></nuxt-link
          >
          <nuxt-link to="/contacts"
            ><p class="text-grayn/50 font-medium text-base mb-7">
              Контакты
            </p></nuxt-link
          >
          <nuxt-link to="/blog"
            ><p class="text-grayn/50 font-medium text-base">Блог</p></nuxt-link
          >
        </div>

        <div class="mb-8 md:mb-0">
          <p class="font-garyn font-bold text-2xl pb-8">Товары</p>
          <p class="text-grayn/50 font-medium text-base mb-7">Люстры</p>
          <p class="text-grayn/50 font-medium text-base mb-7">Светильники</p>
          <p class="text-grayn/50 font-medium text-base mb-7">Бра</p>
          <p class="text-grayn/50 font-medium text-base mb-7">Торшеры</p>
          <p class="text-grayn/50 font-medium text-base mb-7">Комплектуюшие</p>
          <p class="text-grayn/50 font-medium text-base">Настольные лампы</p>
        </div>
        <div class="md:pt-[60px]">
          <p class="text-grayn/50 font-medium text-base mb-7">Споты</p>
          <p class="text-grayn/50 font-medium text-base mb-7">
            Трековые светильники
          </p>
          <p class="text-grayn/50 font-medium text-base mb-7">
            Уличные светильники
          </p>
          <p class="text-grayn/50 font-medium text-base mb-7">
            Технические светильники
          </p>
          <p class="text-grayn/50 font-medium text-base mb-7">
            Светодиодные ленты
          </p>
        </div>
      </div>
      <div class="flex gap-4 items-center">
       <nuxt-link to="https://vk.com/"><img src="/footer/Group 59.png" alt="VK" /></nuxt-link> 
       <nuxt-link to="https://vk.com/"><img src="/footer/Group 59.png" alt="VK" /></nuxt-link>
       <nuxt-link to="https://vk.com/"><img src="/footer/Group 59.png" alt="VK" /></nuxt-link>
      </div>
    </div>
  </div>
</template>

<style scoped></style>
